...
/*
router.post('/adm/lancamentos_filtro', (req,res)=>{
...
router.get('/adm/produtos/:uid_usuario', (req,res)=>{
...
router.get('/adm/addProdutos/:uid_usuario', (req,res)=>{
    const { uid_usuario } = req.params;
...
router.get('/adm/altProduto/:id_produto/:uid_usuario', (req,res)=>{
...
router.post('/adm/produtos', (req,res)=>{
...
router.post('/adm/altProdutos', (req,res)=>{
...
router.get('/adm/remProduto/:id_produto/:uid_usuario', (req,res)=>{
    const { uid_usuario, id_produto } = req.params;
	...
	.where({id_produto: id_produto})
	.del().then(result=>{
		res.redirect('/adm/produtos/'+uid_usuario);
...
router.post('/adm/attDadosPessoais',(req,res)=>{
...
router.get('/adm/principal/:uid_usuario', (req,res)=>{
    ...
    knex('tb_usuario').where({uid_usuario: uid_usuario}).select().then(result=>{		
		...
		knex('tb_empresa').where({id_empresa: id_empresa}).select().then(result_empresa=>{			
			...
			console.log(dataDoDia)
			knex('tb_cliente').where({id_empresa: id_empresa}).select().then(cliente=>{
			knex('tb_lancamento')
			.where({'tb_lancamento.id_empresa': id_empresa})
			.andWhere({'tb_lancamento.data_externa': dataDoDia})
			.leftJoin('tb_cliente','tb_cliente.id_cliente','tb_lancamento.id_cliente')			
			.leftJoin('tb_usuario','tb_usuario.uid_usuario','tb_lancamento.uid_usuario')			
			.select('tb_lancamento.id_lancamento','tb_lancamento.valor','tb_lancamento.data_pagamento_externa',
					...
					knex.raw('CASE WHEN tb_lancamento.data_vencimento_externa="1970-01-01" then "" else tb_lancamento.data_vencimento_externa end  as data_vencimento_externa'),					
					 'tb_lancamento.observacao','tb_lancamento.descricao','tb_lancamento.condicao_pagamento','tb_usuario.descricao as Funcionario').then(lancamentos=>{
					 ...
					 knex('vwVlrTotalReceber').where({id_empresa: id_empresa}).select().then(vlrTotalReceber=>{
						 knex('vwVlrTotalRecebido').where({id_empresa: id_empresa}).select().then(vlrTotalRecebido=>{
							...
							knex('tb_boletos_cobranca').where({id_empresa: id_empresa}).andWhere({status: 0}).andWhere({mostrar_dashboard: 1}).select().then(mostrar_boleto=>{
								...
								.where({'tb_lancamento.id_empresa': id_empresa})
								.andWhere({'tb_lancamento.data_externa': dataDoDia})
								.andWhere({'tb_lancamento.saidas': '1'})
								.sum('valor as valor')
								.select().then(saidas=>{
									console.log('saidas: '+saidas[0].valor)
									...
									valorSaidas : saidas[0].valor == null ? "0" : saidas[0].valor,
...
router.get('/adm/lancamentos/:uid_usuario', (req,res)=>{
    const { uid_usuario } = req.params;
...
router.post('/adm/addLancamento',(req,res)=>{
...
router.post('/adm/editLancamento',(req,res)=>{
	const { uid_usuario, id_cliente,documento, id_empresa , observacao, data, data_vencimento } = req.body;
...
router.get('/adm/verLacamentoCliente/:id_cliente/:uid_usuario', (req,res)=>{
	...
	knex('tb_usuario').where({uid_usuario: uid_usuario}).select().then(result=>{
		...
		.select('tb_lancamento.id_lancamento',
			...
			.then(result=>{
			//console.log(result);
			res.redirect('/adm/lancamentos_cliente/'+id_cliente+'/'+uid_usuario)
		})
	})    
})

router.get('/adm/lancamentos_cliente/:id_cliente/:uid_usuario', (req,res)=>{
	const { uid_usuario, id_cliente } = req.params;
...
router.get('/adm/lancamentos_cliente_detalhes/:id_lancamento/:uid_usuario/:id_cliente?', (req,res)=>{
...
router.get('/adm/remLancamento/:id_lancamento/:uid_usuario', (req,res)=>{
    ...
    knex('tb_usuario').where({uid_usuario: uid_usuario}).select().then(result=>{
		...
		.del()
			.then(result=>{
			res.redirect('/adm/lancamentos/'+uid_usuario)
		})
	})    
})

router.get('/adm/clientes/:uid_usuario', (req,res)=>{
    const { uid_usuario } = req.params;
...
router.get('/adm/addCliente/:uid_usuario', (req,res)=>{
    const { uid_usuario } = req.params;
...
router.post('/adm/addCliente',(req,res)=>{
	...
	.then(result=>{
		...
		knex('tb_cliente').where({id_cliente: id_cliente}).select().then(cliente=>{		
			...
			res.redirect('/adm/addCliente/'+cliente[0].uid_usuario)
		})	
	})
	
})

router.get('/adm/delCliente/:id_cliente/:uid_usuario',(req,res)=>{
    ...
    .then(result=>{
        ...
        res.redirect('/adm/clientes/'+uid_usuario)        
    })
	
})

router.get('/adm/validarCpf/:cpf/:id_empresa',(req,res)=>{
...
router.get('/adm/receberLancamentoParcial/:id_lancamento/:uid_usuario',(req,res)=>{
...
router.get('/adm/receberLancamentoTotal/:id_lancamento/:uid_usuario',(req,res)=>{
	const { id_lancamento, uid_usuario } = req.params;
	knex('tb_lancamento')
	.where({id_lancamento: id_lancamento})
	.update({'status': 1,
	...
	.then(result=>{
		res.redirect('/adm/principal/'+uid_usuario)		
	});
})

router.post('/adm/receberLancamentoParcial',(req,res)=>{
...
router.get('/adm/editLancamento/:id_lancamento/:uid_usuario',(req,res)=>{
	const { id_lancamento, uid_usuario } = req.params;
...
router.get('/adm/pdv/:uid_usuario/:venda', (req,res)=>{
...
router.get('/adm/editpdv/:uid_usuario/:venda', (req,res)=>{
	const { uid_usuario, venda } = req.params;
...
router.get('/adm/vendas/:uid_usuario', (req,res)=>{
    const { uid_usuario } = req.params;
		...
		knex('tb_empresa').where({id_empresa: id_empresa}).select().then(result_empresa=>{			
			...
			knex('tb_cliente').where({id_empresa: id_empresa}).select().then(cliente=>{
				knex('tb_produto').where({id_empresa: id_empresa}).select().then(produtos=>{
					...
					res.redirect('/adm/pdv/'+uid_usuario+'/'+documento_venda())
...
router.get('/adm/getDadosproduto/:id_produto',(req,res)=>{
...
router.post('/adm/produtosVendas',(req,res)=>{
...
router.get('/adm/produtos_tabela/:uid_usuario/:documento_interno/:tipo',(req,res)=>{
...
router.get('/adm/produtos_tabela_total/:documento_interno',(req,res)=>{
...
router.get('/adm/removerProdutosVenda/:id/:uid_usuario/:documento/:tipo', (req,res)=>{
	...
	.del().then(result=>{
		if(tipo=="E"){
			res.redirect('/adm/editPdv/'+uid_usuario+'/'+documento)
		}else{
			res.redirect('/adm/pdv/'+uid_usuario+'/'+documento)
		}
	})	
	
})
/*
router.get('/adm/removerProdutosVenda/:id/:uid_usuario/:documento', (req,res)=>{
	...
	.del().then(result=>{
		...
		//res.redirect('/adm/produtos_tabela/'+documento)
		res.redirect('/adm/pdv/'+uid_usuario+'/'+documento)
